...
export default function TimerScreen() {
  ...
  const startWaitingForMovement = async () => {
    ...
    try {
      locationSubscription.current = await Location.watchPositionAsync(
        ...
        (location) => {
          const speedMph = (location.coords.speed || 0) * 2.237; // Convert m/s to mph
          setCurrentSpeed(Math.max(0, speedMph));
          setGpsAccuracy(location.coords.accuracy);

          // Animate speed dial
          const targetSpeed = modes.find(m => m.type === selectedMode)?.target || 100;
          const speedPercentage = Math.min(speedMph / targetSpeed, 1);
          Animated.timing(speedDialAnim, {
            toValue: speedPercentage,
            duration: 100,
            useNativeDriver: false,
          }).start();

          // Start timer when vehicle starts moving (above 1 mph)
          if (waitingForStart && speedMph > 1 && !isRunning) {
            actuallyStartRun();
          }

          if (isRunning) {
            handleSpeedUpdate(speedMph, location);
          }
        }
      );
    } catch (error) {
  ...
  const handleSpeedUpdate = (speedMph: number, location: Location.LocationObject) => {
    ...
    // Check if target speed reached for 0-60 or 0-100
  ...
  const completeRun = async (time: number, topSpeedResult: number) => {
    ...
    if (isAuthenticated && user) {
      try {
        const runData: any = {
          userId: user.id,
          carInfo: carInfo || `${user.name}'s Car`,
        ...
        Alert.alert(
          '🏆 Run Recorded!',
          `Your ${selectedMode} time of ${time.toFixed(2)}s has been saved to the leaderboard!`,
    ...
    <View style={[styles.container, { paddingTop: insets.top, paddingBottom: insets.bottom }]}>
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.speedometerContainer}>
          <Animated.View
            style={[
              styles.speedometerOuter,
              {
                transform: [{ scale: isRunning || waitingForStart ? pulseAnim : 1 }],
              },
            ]}
          >
            <LinearGradient
              colors={[`${getSpeedColor()}40`, '#1a1a1a']}
        ...
        <View style={styles.gpsStatus}>
          <Ionicons
            name={locationPermission ? 'satellite' : 'satellite-outline'}
            size={16}
