...
async def send_message(message: MessageCreate):
    ...
    return {
        ...
        "senderId": _sid(created_message["senderId"]),
...
async def get_conversations(user_id: str):
    ...
    partner_ids_set = set()
    for msg in messages:
        partner_id = _sid(msg["recipientId"]) if _sid(msg["senderId"]) == user_id else _sid(msg["senderId"])
        partner_ids_set.add(partner_id)

    # Batch fetch all partners in one query (fixes N+1)
    if partner_ids_set:
        partners_list = await db.users.find(
            {"_id": {"$in": [ObjectId(pid) for pid in partner_ids_set if ObjectId.is_valid(pid)]}},
    ...
    for msg in messages:
        ...
        if partner_id in conversations and _sid(msg["recipientId"]) == user_id and not msg.get("isRead", False):
