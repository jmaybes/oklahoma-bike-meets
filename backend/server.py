...
def event_photo_helper(photo) -> dict:
    return {
        ...
        "tags": photo.get("tags", []),  # List of {userId, carId, carInfo}
...
@api_router.post("/events/{event_id}/gallery/{photo_id}/tag")
...
@api_router.delete("/events/{event_id}/gallery/{photo_id}/tag/{car_id}")
async def remove_car_tag(event_id: str, photo_id: str, car_id: str, user_id: str = Query(...)):
    """Remove a car tag from a photo (only the car owner can remove their tag)"""
    if not ObjectId.is_valid(photo_id):
        raise HTTPException(status_code=400, detail="Invalid photo ID")
...
@api_router.post("/events/{event_id}/gallery/{photo_id}/like")
...
async def delete_event_photo(event_id: str, photo_id: str, user_id: str = Query(...)):
    """Delete a photo from event gallery (only uploader or admin can delete)"""
    if not ObjectId.is_valid(photo_id):
        raise HTTPException(status_code=400, detail="Invalid photo ID")
